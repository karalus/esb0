...
public class SchemaWriter implements XSVisitor, XSSimpleTypeVisitor {
	...
	private Function<String, String> uri2filename;
	public void setUri2filenameFunction(Function<String, String> uri2filename) {
		this.uri2filename = uri2filename;
	}
    ...
    public void schema( XSSchema s ) {
        ...
        if (namespaceMap != null) {
            for (Map.Entry<String, String> entry : namespaceMap.entrySet()) {
    			nsDecls.append(" xmlns:").append(entry.getValue()).append("=\"").append(entry.getKey()).append('"');
    		}
        }
        
        println(MessageFormat.format("<schema xmlns=\"http://www.w3.org/2001/XMLSchema\"{1} targetNamespace=\"{0}\" elementFormDefault=\"qualified\">", s.getTargetNamespace(), nsDecls));
        indent++;

        if (namespaceMap != null) {
            for (String namespace : namespaceMap.keySet()) {
            	if (!namespace.equals(s.getTargetNamespace())) {
            		String schemaLocation = "";
            		if (uri2filename != null) {
                		String filename = uri2filename.apply(namespace);
                		if (filename != null) {
                			schemaLocation = " schemaLocation=\"" + filename + ".xsd\"";
                		}
            		}
                	println(MessageFormat.format("<import namespace=\"{0}\"{1}/>", namespace, schemaLocation));
            	}
    		}
        }
    ...
    public void dumpRef( XSAttGroupDecl decl ) {
        println(MessageFormat.format("<attributeGroup ref=\"{0}{1}\"/>", getPrefix(decl.getTargetNamespace()), decl.getName()));
    }
    ...
    public void attributeUse( XSAttributeUse use ) {
        ...
        if(decl.isLocal()) {
            ...
            println(MessageFormat.format("<attribute ref=\"{0}{1}{2}\"/>",
            	getPrefix(decl.getTargetNamespace()), decl.getName(), additionalAtts));
        }
    }
    ...
    private void dump( XSAttributeDecl decl, String additionalAtts ) {
        ...
        println(MessageFormat.format("<attribute name=\"{0}\"{1}{2}{3}{4}{5}>",
            ...
            additionalAtts,
            type.isLocal()?"":
                MessageFormat.format(" type=\"{0}{1}\"", getPrefix(type.getTargetNamespace()), type.getName()),
    ...
    public void listSimpleType( XSListSimpleType type ) {
        ...
        if(itemType.isLocal()) {
            ...
            println(MessageFormat.format("<list itemType=\"{0}{1}\" />",
            	getPrefix(itemType.getTargetNamespace()), itemType.getName()));
        }
    }
    ...
    public void unionSimpleType( XSUnionSimpleType type ) {
        ...
        for( int i=0; i<len; i++ ) {
            ...
            if(member.isGlobal())
                ref.append(MessageFormat.format(" {0}{1}", getPrefix(member.getTargetNamespace()),member.getName()));
        }
    ...
    public void restrictionSimpleType( XSRestrictionSimpleType type ) {
        ...
        println(MessageFormat.format("<restriction{0}>",
            ...
            getPrefix(baseType.getTargetNamespace())+
            baseType.getName()+'\"'));
        indent++;
        ...
        if(type.getContentType().asSimpleType()!=null) {
            ...
            if(type.getDerivationMethod()==XSType.RESTRICTION) {
                ...
                println(MessageFormat.format("<restriction base=\"{0}{1}\">",
                	getPrefix(baseType.getTargetNamespace()), baseType.getName()));
                indent++;
            ...
            boolean notAnyType = baseType.getBaseType() != baseType;
			if (notAnyType) {
	            // complex content
    ...
    private void elementDecl( XSElementDecl decl, String extraAtts ) {
        ...
        println(MessageFormat.format("<element name=\"{0}\"{1}{2}{3}>",
            decl.getName(),
            type.isLocal()?"":" type=\""+
            getPrefix(type.getTargetNamespace())+
            type.getName()+'\"',
            extraAtts,
            type.isLocal()?"":"/"));
            ...
            public void elementDecl( XSElementDecl decl ) {
                if(decl.isLocal())
                    ...
                    println(MessageFormat.format("<element ref=\"{0}{1}\"{2}/>",
                    	getPrefix(decl.getTargetNamespace()),
                        decl.getName(),
                        extraAtts));
                }
            }
