	...
	protected void validateInternal(GlobalContext globalContext) throws Exception {
		...
		try {
			...
			case HTTP:
				...
				_consumerPort = new HttpConsumer(getURI(), globalContext.bindProperties(httpBinding.getValue()), httpBinding.getMinPool(), httpBinding.getMaxPool(), httpBinding.getKeepAlive(),
						httpBinding.isSupportCompression(), httpBinding.getMultipartResponse(), httpBinding.getBufferSize());
				break;
			case JMS:
